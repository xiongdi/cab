...
use support::local_uat::{
    AUTO_STRATEGIES, ClaudeSettingsBackup, assert_all_agents_mode, assert_anthropic_text,
    assert_assistant_text, connect_packaged_server, post_chat_completions, post_messages,
    put_agent_auto, restore_agents, set_all_agents_mode, snapshot_agents,
};
...
async fn uat_02_auto_strategy_routes_real_request() {
    uat!(
        "UAT-02",
        "Auto strategy (balanced) routes real request",
        |guard| {
            let server = connect_packaged_server().await;
...
async fn uat_03_active_providers_from_user_config() {
    uat!(
        "UAT-03",
        "Dashboard active providers from user config",
        |guard| {
            let server = connect_packaged_server().await;
            support::local_uat::first_routable_model(&server).await;
...
async fn uat_05_real_requests_logged_and_filterable() {
    uat!("UAT-05", "Request logs filterable and paginated", |guard| {
        ...
        let data = filtered
            ...
            .expect("log data");
        guard.note(&format!(
            "total_logs={total}, claude-code_logs={}",
            data.len()
        ));
        assert!(total >= 3);
...
async fn uat_06_claude_auto_writes_real_settings_json() {
    uat!(
        "UAT-06",
        "Claude Code auto writes ~/.claude/settings.json",
        |guard| {
...
async fn uat_08_anthropic_messages_when_available() {
    uat!(
        "UAT-08",
        "Anthropic Messages API (provider anthropic endpoint)",
        |guard| {
            let server = connect_packaged_server().await;
...
async fn uat_10_all_cas_real_chat_completion() {
    uat!(
        "UAT-10",
        "All coding agents (7 CAs) real CLI completion",
        |guard| {
            let server = connect_packaged_server().await;
            ...
            for agent_id in SUPPORTED_AGENT_IDS {
                ...
                let result = run_real_ca(&server, agent_id, "balanced", UAT_CA_PROMPT).await;
                results.push(result);
            }
...
async fn uat_11_all_agents_auto_mode_real_requests() {
    uat!(
        "UAT-11",
        "All agents auto mode × 4 strategies (real CLIs)",
        |guard| {
            let _backup = ClaudeSettingsBackup::new();
            let server = connect_packaged_server().await;
            let snapshot = snapshot_agents(&server).await;
            ...
            for strategy in AUTO_STRATEGIES {
                ...
                for agent_id in SUPPORTED_AGENT_IDS {
                    let result = run_real_ca(&server, agent_id, strategy, UAT_CA_PROMPT).await;
...
async fn uat_12_all_agents_manual_mode_real_requests() {
    uat!(
        "UAT-12",
        "All agents manual mode real CLI requests",
        |guard| {
            let _backup = ClaudeSettingsBackup::new();
            let server = connect_packaged_server().await;
            let snapshot = snapshot_agents(&server).await;
            let (_provider_id, model_name) =
                support::local_uat::first_routable_model(&server).await;
            ...
            let mut results = Vec::new();
            for agent_id in SUPPORTED_AGENT_IDS {
                let result = run_real_ca(&server, agent_id, &model_name, UAT_CA_PROMPT).await;
                results.push(result);
            }
            restore_agents(&server, &snapshot).await;
            assert_all_real_ca_passed(&results);
