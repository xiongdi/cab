...
**路由解析**：`resolve_route(pool, agent, model, body)` → `ResolvedRoute`。

## 管理 API 契约（`/api`）

完整路由见 `cab-api/src/lib.rs`。统一响应：

- 成功：`200` + JSON 实体
- 未找到：`CabError::NotFound` → 4xx
- 数据库错误：`CabError::Database`

CORS：`CorsLayer` 允许任意 Origin（本地管理 UI）。
...
- 读：`inner.read()`
- 写：`inner.write()` + 必要时 `settings::save_to_disk`
