...
CAB (Coding Agents Bridge) 是一个面向编码代理和开发者工作流的本地、成本感知型 LLM 网关路由器。将你的代理 CLI 指向 CAB 网关（默认 `http://localhost:3125/v1`）；CAB 会根据每个提示词选择最佳的已启用供应商和模型并转发请求。

---

## 功能

- **OpenAI / Anthropic / Gemini 网关**：在同一个本地 HTTP 端口暴露 `/v1/chat/completions`、`/v1/messages`、`/v1/responses` 以及 Gemini 兼容端点。
- **能力与成本感知路由**：根据 Intelligence / Coding / Agentic 指标、token 价格和上下文窗口对模型排序。
- **实时目录同步**：从 `models.dev` 拉取模型、价格和基准数据。
- **桌面仪表盘**：基于 Tauri + Svelte 的 UI，用于管理供应商、密钥、路由策略、代理配置和请求日志。
