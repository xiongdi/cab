...
CAB 按配置顺序与 Key 可用性（含 429 冷却）选择要使用的密钥。上游返回 **429** 时，会先对同一 Key 做指数退避重试（若有较短的 `Retry-After` 会遵守）；重试耗尽后再短暂冷却该 Key，并 fallback 到下一个 Key 或模型。
...
models.dev 会把同一模型挂在不同提供商下，并带上各自的端点价格（例如按量 `minimax` 与套餐 `minimax-cn-coding-plan`）。**不同计费方式请启用对应的 models.dev 提供商并配置 Key**；路由按各提供商的端点价格与策略评分排序，不再在 Key 上区分「订阅 / 按量」。
