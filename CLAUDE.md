...
**CAB (Coding Agents Bridge)** — a local, cost-aware LLM gateway router for coding agent CLIs. Agents point at `http://localhost:3125/v1`; CAB ranks providers/models (Intelligence / Coding / Agentic indices + token pricing + context window) and forwards requests upstream to OpenAI / Anthropic. Syncs model/pricing/benchmark data from `models.dev` in real time. Ships a Svelte browser dashboard (served by the gateway) for managing providers, keys, routing strategies, and per-agent config (Claude Code, Codex, OpenCode, Grok Build).

## Workspace layout

Cargo workspace (`crates/*`) + Svelte frontend (`src/`).
...
| `cab-srv`      | Library: combined HTTP app (gateway + API + static UI) (`crates/cab-server`)                                   |
| `cab`          | Single binary `cab` (`serve` / service control / `gui` / `update`)                                             |
...
- **Forbidden:** stacking a second gateway on 3125 (`cab serve` / release `cab` vs `dev:server`), `npm run dev:server:once` for daily work, or changing ports to dodge a conflict.
...
npm run dev:server                 # cargo watch → cab serve on :3125

# Foreground server only (NOT for daily dev — conflicts with dev:server)
...
1. Clean: `scripts/kill-dev-ports.ps1` + stop stray `claude,cab,cab-srv,cargo-watch`.
...
`Cargo.toml` `[workspace.package]` `version` and `package.json` `version` move together (currently **0.9.0**). Releases via GitHub Releases; `CHANGELOG.md` is maintained per release. PRs target `main`. See `CONTRIBUTING.md` and `.github/PULL_REQUEST_TEMPLATE.md`.
