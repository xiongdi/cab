...
`Cargo.toml` `[workspace.package]` `version` and `package.json` `version` move together (currently 0.2.7). Releases via GitHub Releases; `CHANGELOG.md` is maintained per release. PRs target `main`. See `CONTRIBUTING.md` and `.github/PULL_REQUEST_TEMPLATE.md`.
